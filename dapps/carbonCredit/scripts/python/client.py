...
tx_veiculo = contract.functions.registrarVeiculo(account.address, 1234, 1, 2841082).build_transaction({
    'from': account.address,
    'nonce': nonce_veiculo,
    'gas': 200000,  # Aumentando o limite de gás
    'gasPrice': w3.eth.gas_price,
})
...
print("Hash da transação do veículo:", tx_hash_veiculo.hex())
...
# Obter novo nonce APÓS a confirmação da primeira transação
...
tx_viagem = contract.functions.registrarViagem(account.address, 1234, 30, 1000).build_transaction({
    'from': account.address,
    'nonce': nonce_viagem,
    'gas': 300000,  # Aumentando o limite de gás para esta transação
    'gasPrice': w3.eth.gas_price,
})
...
print("Hash da transação da viagem:", tx_hash_viagem.hex())

# Aguardar a confirmação da transação de registro da viagem
receipt_viagem = w3.eth.wait_for_transaction_receipt(tx_hash_viagem)

print("Verificando tokens disponíveis para saque...")
try:
    tokens_disponiveis = contract.functions.tokensDisponiveisParaSaque(account.address).call()
    print(f"Tokens disponíveis para saque: {tokens_disponiveis}")
except Exception as e:
    print(f"Erro ao verificar tokens disponíveis: {e}")

# Processar o evento para obter o tokenId
# print("Buscando o tokenId do recibo da transação...")
# events = contract.events.ViagemRegistrada().process_receipt(receipt_viagem)
# print(events)
