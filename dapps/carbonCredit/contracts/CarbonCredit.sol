...
contract CarbonCreditNFT is ERC721, Ownable, ReentrancyGuard {
    uint256 public carbonPricePerG; // // em weight
    ...
    enum Combustivel { Gasolina, Etanol, Diesel, Eletrico }
    ...
    event ViagemRegistrada(uint256 tokenId, address condutor, uint256 co2RealG, uint256 co2MetaG, uint256 recompensa, uint256 economia);
    event RecompensaSacada(uint256 tokenId, address condutor, uint256 valor);
    event VeiculoRegistrado(address condutor, uint256 idVeiculo, Combustivel combustivel, uint256 fator);
    event PrecoCarbonoAtualizado(uint256 novoPrecoWei, uint256 centavos, uint256 cotacao);


    constructor(uint256 _centavosPorG, uint256 _cotacaoInicial, address initialOwner)
    ...
    function registrarVeiculo(address _condutor, uint256 idVeiculo, Combustivel _combustivel, uint256 _fatorGCO2PorKm) external onlyOwner {
        ...
        emit VeiculoRegistrado(_condutor, idVeiculo, _combustivel, _fatorGCO2PorKm);
    }


    function registrarViagem(address _condutor, uint256 idVeiculo, uint256 _distanciaKm, uint256 _co2RealG) external onlyOwner returns (uint256) {
        require(_distanciaKm > 0, "Distancia deve ser positiva");
        require(_co2RealG >= 0, "CO2 real invalido");
        require(veiculos[_condutor][idVeiculo].registrado, "Veiculo nao registrado");
        ...
        emit ViagemRegistrada(tokenId, _condutor, _co2RealG, co2MetaG, recompensa, economia);
        return tokenId;
    }
    ...
    function tokensDisponiveisParaSaque(address _condutor) external view returns (uint256[] memory) {
        require(msg.sender == _condutor || msg.sender == owner(), "Apenas o proprio condutor ou o dono do contrato podem consultar");


        uint256[] memory todos = tokensDoCondutor[_condutor];
        uint256 count;
        for (uint256 i = 0; i < todos.length; i++) {
            if (!viagemInfo[todos[i]].recompensaSacada && viagemInfo[todos[i]].recompensa > 0) {
                count++;
            }
        }
    ...
    function atualizarCotacaoEth(uint256 novaCotacaoEmReais) external onlyOwner {
    ...
    function atualizarPrecoCentavos(uint256 novoPrecoCentavos) external onlyOwner {
