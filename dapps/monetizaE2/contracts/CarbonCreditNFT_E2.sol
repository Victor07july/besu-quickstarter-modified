...
contract CarbonCreditNFT_E2Calculator is ERC721, ERC721Enumerable, ReentrancyGuard, Ownable {
        ...
        returns (CalculationResult memory result) 
    {
        ...
        uint256 convertGasoline = (result.tanqueGasoline * 6339906) / (100 * 1e6);
        ...
        if (params.roadGasoline > 0) {
            result.dtEstradaGasolina = (convertGasoline * params.highwayDistance) / params.roadGasoline;
        }
        if (params.roadEthanol > 0) {
            result.dtEstradaEtanol = (convertEtanol * params.highwayDistance) / params.roadEthanol;
        }
        ...
        if (params.cityGasoline > 0) {
            result.dtCidadeGasolina = (convertGasoline * params.cityDistance) / params.cityGasoline;
        }
        if (params.cityEthanol > 0) {
            result.dtCidadeEtanol = (convertEtanol * params.cityDistance) / params.cityEthanol;
        }
        ...
        result.propBonus = 
            (params.behaviorCautious * 50000) / 1e6 +      // 0.05 = 50000/1e6
            (params.behaviorNormal * 20000) / 1e6 +        // 0.02 = 20000/1e6
            (params.behaviorAggressive * 5000) / 1e6;      // 0.005 = 5000/1e6
